...
exports.register = function (server, options, next) {
  ...
  server.route({
    ...
    path: '/suppliers',
    handler: (request, reply) => {
      const get = request.query.deliver_to ? Supplier.findByDeliveryPostcode(request.query.deliver_to) : Supplier.find();
    ...
    config: {
      tags: ['api'],
      pre: [{method: getDeliveryPostcodeData}],
      validate: {
        query: {
          deliver_to: Joi.string().regex(/[A-Z]{1,2}[0-9][0-9A-Z]?[0-9][A-Z]{2}/i, 'postcode').description('Delivery postcode')
        }
      },
      response: {
        failAction: process.env.RESPONSE_FAIL_ACTION || 'log',
