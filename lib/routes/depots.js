...
const postcodeRegexes = require('../postcode_regexes');
...
const depotParametersSchema = Joi.object({
  ...
  delivery_outward_codes: Joi.array().required().items(Joi.string().regex(postcodeRegexes.outwardCode, 'outward code')).description('Outward codes where deliveries can be made - https://en.wikipedia.org/wiki/Postcodes_in_the_United_Kingdom#Outward_code'),
  delivery_postcode_areas: Joi.array().required().items(Joi.string().regex(postcodeRegexes.postcodeArea, 'postcode area')).description('Postcode areas where deliveries can be made - https://en.wikipedia.org/wiki/Postcodes_in_the_United_Kingdom#Postcode_area')
}).meta({
